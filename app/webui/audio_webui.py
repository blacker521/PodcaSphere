...
class AudioWebUI:
    def __init__(self, configs):
        ...
        with gr.Row():
            with gr.Column():
                ...
                save_path = gr.Textbox(value="tmp/", label="保存路径")
                save_button = gr.Button("2. 保存发言稿")
            with gr.Row():
                json_table = gr.DataFrame(value=pd.DataFrame(self.speech_json["outputs"]), label="编辑表格")

        speech_button.click(fn=self.get_speech, inputs=[audio_path],
                            outputs=[json_table])  # 更新表格输出
        save_button.click(fn=self.save_speech, inputs=[json_table, save_path, audio_path])  # 更新表格输出
    ...
    def save_speech(self, json_table, save_path, audio_path):
        try:
            file_name = os.path.splitext(os.path.basename(audio_path))[0]
            self.speech_json['file_name'] = file_name
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            with open(f'{save_path}/{file_name}_speech.json', 'w', encoding='utf-8') as f:
